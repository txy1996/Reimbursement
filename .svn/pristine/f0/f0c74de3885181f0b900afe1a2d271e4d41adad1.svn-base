			...
			try {
				...
				if(field.isAnnotationPresent(Id.class)){
	                ...
	                idFieldValue = pd.getReadMethod().invoke(t); 
						...
						fieldNames.add(field.getAnnotation(Number.class).value());  
		...
		for(Field file:fiel){
			try {
				...
				if(file.isAnnotationPresent(Id.class)){
					...
					placeHolders.append("?").append(","); 
				}else if(file.isAnnotationPresent(Number.class)){
					fieldNames.append(file.getAnnotation(Number.class).value()).append(",");
					try {
						values.add(pd.getReadMethod().invoke(t));
					} catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
						// TODO Auto-generated catch block
	 ...
	 private void initObject(T t, Field[] fields, ResultSet rs)  
	        ...
	        for (Field field : fields) { //���forѭ���������ڰ�ֵ����ʵ�����С�
	            ...
	            if(field.isAnnotationPresent(Id.class)){
	            	...
	            	columnName = field.getAnnotation(Number.class).value();
	            }
