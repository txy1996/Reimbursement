...
public class FinanceDaoImpl extends CurrencyDaoImpl<Finance> implements FinanceDao {
	...
	public List<Finance> FinanceFindAll() {
		...
		try {
			finance = DBUtil.getQueryRunner().query(sql, new BeanListHandler<>(Finance.class));
	...
	public Finance FinanceFindById(int id) {
		...
		try {
			finance = DBUtil.getQueryRunner().query(sql, new BeanHandler<>(Finance.class), id);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public int FinanceDelete(int id) {
		...
		int a = 0;
		try {
			a = DBUtil.getQueryRunner().update(sql, id);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public int FinanceUpdate(Finance finance) {
		...
		int a = 0;
		try {
			a = DBUtil.getQueryRunner().update(sql, finance.getApp_abb(), finance.getFinance_status(),
 ...
 */
	...
	public void FinanceAdd(ApplyreimVo apply) {
		// TODO Auto-generated method stub
		String sql="INSERT INTO finance(app_abb,finance_money,finance_date,finance_attn) VALUES\r\n" + 
				"(?,?,?,?)";
		QueryRunner runner = new QueryRunner();
		
		try {
			runner.update(DBUtil.getConnection(), sql, apply.getApp_abb(),apply.getMoney(),apply.getTime(),apply.getEmp_name());
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
