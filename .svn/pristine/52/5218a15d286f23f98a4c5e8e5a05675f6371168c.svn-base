...
public class ReimnumtabDaoImpl extends CurrencyDaoImpl<Reimnumtab> implements ReimnumtabDao{
	...
	public ApplyreimVo FindById(Reimnumtab reimnumtab) {
		...
		ApplyreimVo apply=new ApplyreimVo();
		try {
	...
	public GetMaxMoneyVo findLevel(String name) {
		String sql = "SELECT level.`level_money` AS money FROM emp,posi,LEVEL "
				+" WHERE emp.`post_id` = posi.`position_id` AND"
				+" posi.`position_level` = level.`position_level`"
				+" AND emp.emp_name = ?";
		GetMaxMoneyVo money = null;
		try {
			money = DBUtil.getQueryRunner().query(sql, new BeanHandler<GetMaxMoneyVo>(GetMaxMoneyVo.class),name);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
